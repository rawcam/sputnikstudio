// led.js (исправленная версия с корректным округлением пикселей)
const LedModule = (function() {
    ...
    // Функция для получения целого числа пикселей в кабинете (округление вниз)
        ...
        if (mode === 'cabinets') {
            ...
            html = `
                <div class="calc-card">
                    ...
                    <div class="ergo-info">Система подбирает ближайшее количество кабинетов, покрывающее заданное разрешение, используя целое число пикселей в кабинете (округляя вниз).</div>
                </div>
            `;
    ...
    function updateCalculations(mode, skipSetState = false) {
        ...
        if (mode === 'cabinets') {
            ...
            // Исправление: округление вниз
            const { pixelsW, pixelsH } = getCabPixels(cabW_mm, cabH_mm, pitch);
            let cW = Math.ceil(targetW / pixelsW);
            let cH = Math.ceil(targetH / pixelsH);
            if (cW < 1) cW = 1;
            if (cH < 1) cH = 1;

            const realResW = pixelsW * cW;
            const realResH = pixelsH * cH;
            const width_m = (cW * cabW_mm) / 1000;
            const height_m = (cH * cabH_mm) / 1000;
            const area = width_m * height_m;
            const power = area * powerPerSqm;
